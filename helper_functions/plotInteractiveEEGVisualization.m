function plotInteractiveEEGVisualization(eeg_data, spect2, AMI, SMI, trimmed_time, fs, win_length, fmax, cmin, cmax, threshold_type, AMI_threshold, SMI_threshold, baseline_start_idx, baseline_end_idx, aperiodic_data, dataset_name)
...
fig = figure('Name', sprintf('EEG Analysis: %s', dataset_name), 'Position', [50, 50, 1500, 900], 'units', 'normalized', 'MenuBar', 'none', 'ToolBar', 'figure');
...
control_panel = uipanel('Parent', fig, ...
                        ...
                        'Position', [0.85, 0.70, 0.14, 0.25], ... 
                        'FontWeight', 'bold');

colorscale_panel = uipanel('Parent', fig, ...
                        'Title', 'Spectrogram Controls', ...
                        'Position', [0.85, 0.50, 0.14, 0.20], ... 
                        'FontWeight', 'bold');

util_panel = uipanel('Parent', fig, ...
                    'Title', 'Utilities', ...
                    'Position', [0.85, 0.25, 0.14, 0.25], ... 
                    'FontWeight', 'bold');

aperiodic_panel = uipanel('Parent', fig, ...
                    'Title', 'Aperiodic Analysis', ...
                    'Position', [0.85, 0.15, 0.14, 0.10], ... 
                    'FontWeight', 'bold');

extraction_panel = uipanel('Parent', fig, ...
                         'Title', 'Extract EEG Segment', ...
                         'Position', [0.85, 0.01, 0.14, 0.15], ... 
                         'FontWeight', 'bold');

% Extraction Panel
% Start time input
uicontrol('Parent', extraction_panel, ...
          'Units', 'normalized', ...
          'Style', 'text', ...
          'String', 'Start (min):', ...
          'Position', [0.05, 0.8, 0.4, 0.15]);

extract_start = uicontrol('Parent', extraction_panel, ...
                          'Units', 'normalized', ...
                          'Style', 'edit', ...
                          'String', num2str(min(trimmed_time), '%.2f'), ...
...
extract_end = uicontrol('Parent', extraction_panel, ...
                         'Units', 'normalized', ...
                         'Style', 'edit', ...
                         'String', num2str(max(trimmed_time), '%.2f'), ...
                         'Position', [0.5, 0.6, 0.4, 0.15]);
...
uicontrol('Parent', extraction_panel, ...
          ...
          'Style', 'text', ...
          'String', 'Filename:', ...
          'Position', [0.05, 0.4, 0.4, 0.15]);

extract_filename = uicontrol('Parent', extraction_panel, ...
                             'Units', 'normalized', ...
                             'Style', 'edit', ...
                             'String', ['segment_' strrep(dataset_name, '.mat', '')], ...
                             'Position', [0.5, 0.4, 0.45, 0.15]);

% Extract button
extract_button = uicontrol('Parent', extraction_panel, ...
                           'Units', 'normalized', ...
                           'Style', 'pushbutton', ...
                           'String', 'Extract', ...
                           'Position', [0.3, 0.1, 0.4, 0.2], ...
                           'Callback', @extractEEGSegmentHandler);  % Make sure this matches your callback name

% Store extraction controls in UserData for access in callback
fig.UserData.extract_start = extract_start;
fig.UserData.extract_end = extract_end;
fig.UserData.extract_filename = extract_filename;

% Add "set to current view" button
set_view_button = uicontrol('Parent', extraction_panel, ...
                           'Units', 'normalized', ...
                           'Style', 'pushbutton', ...
                           'String', 'Set to View', ...
                           'Position', [0.05, 0.1, 0.2, 0.2], ...
                           'Callback', @setExtractionToViewHandler);  % Make sure this matches your callback name

% Add callbacks to update duration when extract times change
set(extract_start, 'Callback', @updateExtractDuration);  % Make sure this matches your callback name
set(extract_end, 'Callback', @updateExtractDuration);    % Make sure this matches your callback name
...
% Spectrogram color control panel 
...
uicontrol('Parent', util_panel, ...
          ...
          'Position', [0.05, 0.4, 0.45, 0.08]);

low_cutoff = uicontrol('Parent', util_panel, ...
                       'Units', 'normalized', ...
                       'Style', 'edit', ...
                       'String', '8', ...
                       'Position', [0.52, 0.4, 0.2, 0.08], ...
                       'Tag', 'LowCutoffEdit');
...
% Callback for extracting EEG segment
function extractEEGSegmentHandler(~, ~)
    % Get values from UI controls
    start_time = str2double(fig.UserData.extract_start.String);
    end_time = str2double(fig.UserData.extract_end.String);
    filename = fig.UserData.extract_filename.String;
    
    % Validate inputs
    if isnan(start_time) || isnan(end_time) || start_time >= end_time
        errordlg('Please enter valid start and end times.', 'Input Error');
        return;
    end
    
    updateStatus('Extracting EEG segment...', fig);
    
    try
        % Call the extraction function
        extractEEGSegment(fig.UserData.eeg_data, fig.UserData.trimmed_time, ...
                          fig.UserData.fs, start_time, end_time, filename);
        updateStatus('EEG segment extracted successfully', fig);
    catch e
        errordlg(['Error extracting data: ' e.message], 'Extraction Error');
        updateStatus(['Error: ' e.message], fig);
    end
end

% Callback to set extraction time range to current view
function setExtractionToViewHandler(~, ~)
    % Get current x-axis limits from Tab 1
    current_xlim = get(fig.UserData.ax1, 'XLim');
    
    % Update extraction time range fields
    fig.UserData.extract_start.String = num2str(current_xlim(1), '%.2f');
    fig.UserData.extract_end.String = num2str(current_xlim(2), '%.2f');
    
    updateStatus('Extraction range set to current view', fig);
end

% Function to update extraction duration when start/end times change
function updateExtractDuration(~, ~)
    start_time = str2double(fig.UserData.extract_start.String);
    end_time = str2double(fig.UserData.extract_end.String);
    
    if ~isnan(start_time) && ~isnan(end_time) && start_time < end_time
        duration = end_time - start_time;
        updateStatus(sprintf('Extraction duration: %.2f min (%.1f samples)', duration, duration*60*fig.UserData.fs), fig);
    end
end
end